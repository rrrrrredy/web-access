...
必需输入：目标网站或起始页面、用户要完成的具体目标、是否允许使用登录态或执行有副作用操作。

默认输出：

- 已访问的页面或数据来源；
- 使用的访问方式（WebSearch/WebFetch/curl/CDP/Jina）；
- 关键发现或已完成操作；
- 证据链接、截图路径或 DOM 证据；
- 未完成原因、风控/登录墙/权限限制和下一步建议。

不得输出 cookie、token、密码、验证码、支付信息、私信、通讯录或与任务无关的个人信息。
...
很多网站的链接包含导航所需的查询参数。打开页面时保留原始完整 URL，不裁剪参数；但不要单独读取、转述、记录或导出疑似凭证、会话、支付或个人身份参数。
